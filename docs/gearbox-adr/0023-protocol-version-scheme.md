...
- Failure mode of forgetting to tag: the next PR's author, using "the latest tag at merge time" as its base, will naturally catch up the gap (taking the heavier of the two PRs' segments — the **max**, not the sum: stacked pending bumps of the same segment collapse into a single bump, e.g. two untagged minors from v1.3.0 target one v1.4.0, not v1.5.0; clarified via #69), so the version number stays monotonic; if a missed tag is discovered, open a Protocol gap issue per the protocol
