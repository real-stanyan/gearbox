...
| 下游（downstream） | 拷走本 scaffold 协议后独立演进的项目；权威清单在 `DOWNSTREAM.md`「已接入项目」 | 入清单三条件见 DOWNSTREAM.md |
| 回流（backfill） | scaffold 协议改进通过在下游 repo 开提醒 issue 传达给下游；是提醒不是强制，下游可拒（关 issue 注明理由） | ADR-0013（B-3 模板强制） |
